...
# Load data----
# https://cellxgene.cziscience.com/collections, UMAP of Myeloids, Seurat v4
seu <- readRDS("RDSfiles/Myeloids.rds")
seu
View(seu@meta.data)
DefaultDimReduc(seu)
...
table(seu$tissue) # body of stomach includes 6150 cells
...
FeaturePlot(seu,features = "FCN1", cols = c("lightgrey","darkred"), label = TRUE, repel = TRUE) + NoAxes() + NoLegend()
FeaturePlot(seu,features = "KIT", cols = c("lightgrey","darkred"), label = TRUE, repel = TRUE) + NoAxes() + NoLegend()

saveRDS(seu, file = "RDSfiles/Myeloids_annotated.RDS")
...
write.table(propres, file = "results/DA//propeller_Myeloids_ARID1AHvsL.txt", sep ="\t", col.names = T,row.names = F)
plotCellTypeProps(clusters = seu$cell_type, sample = seu$donor_id)

# remove samples which contain less than 100 cells
table(seu$donor_id)
seu <- subset(seu, donor_id %in% c("L01", "L09", "P6592", "P6649", "Patient16", "Patient17", "Patient22"), invert = TRUE)
table(seu$donor_id)
propres <- propeller(clusters = seu$cell_type, sample = seu$donor_id, group = seu$ARID1A_level)
write.table(propres, file = "results/DA//propeller_Bcells_ARID1AHvsL_2.txt", sep ="\t", col.names = T,row.names = F)
plotCellTypeProps(clusters = seu$cell_type, sample = seu$donor_id)
